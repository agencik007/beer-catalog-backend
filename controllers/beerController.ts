...
// @route  GET /api/beers
// @access Private
export const getBeers = asyncHandler(
...
// @access Private
export const userBeers = asyncHandler(
	async (req: Request, res: Response, next: NextFunction) => {
		const { page = 1, limit = 100 } = req.query;
...
export const updateBeer = asyncHandler(async (req: Request, res: Response) => {
	...
	if (beer.user.toString() !== decodeUserId.toString()) {
		...
		throw new ValidationError(
			'Only the beer creator can edit the beer entry.'
		);
	} else {
...
export const deleteBeer = asyncHandler(async (req: Request, res: Response) => {
	...
	if (beer.user.toString() !== decodeUserId.toString()) {
		res.status(401);
		throw new ValidationError(
			'Only the beer creator can delete the beer entry.'
		);
	} else {
		await beer.remove();
